...
def get_escential_data(current_btc_price, amount, market):
    ...
    if current_btc_price == 0:
        current_btc_price = Decimal(current_stats(market)["ask"])
    else:
        print(current_btc_price)

    result = show_help_message(
        "Entrada con " + f"{Decimal(amount):,}",
        " a " + f"{Decimal(current_btc_price):,}",
    )
    ...
    stop_lost = round(Decimal(amount) * Decimal(0.01), 2) * -1
    d = b
    price = current_btc_price
    ...
    result = result + show_help_message(
        "Stop Loss -1%: " + str(price) + " con un Stop Lost de $", str(stop_lost)
    )
    result = result + print_values(d)

    stop_lost = round(Decimal(amount) * Decimal(0.02), 2) * -1
    d = b
    price = current_btc_price
    ...
    result = result + show_help_message(
        "Stop Loss -2%: " + str(price) + " con un Stop Lost de $", str(stop_lost)
    )
    result = result + print_values(d)
